function fft_fcn(f,date,dt,thold,varargin)
...
addOptional(p,'freqlim',[])
addOptional(p,'datelim',[])

parse(p,f,date,dt,thold,varargin{:})
%
Parent = p.Results.Parent;
freqlim = p.Results.freqlim;

datelim =  p.Results.datelim;
%%
...
plot(freq(L),log(PSD(L)),'r','LineWidth',1.5), hold on
plot(freq(L),log(PSDclean(L)),'-.b','LineWidth',1.2)
legend('Noisy','Filtered')
